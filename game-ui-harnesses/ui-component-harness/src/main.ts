...
async function exportBundle() {
  ...
  if (contract.schemaVersion === '0.1') needed.add(contract.slots.visual.props.source);
    ...
    if (node.type === 'List' && node.props.appearance) { if(node.props.appearance.backgroundImage) needed.add(node.props.appearance.backgroundImage); needed.add(node.props.appearance.rowImage); needed.add(node.props.appearance.selectedRowImage); }
