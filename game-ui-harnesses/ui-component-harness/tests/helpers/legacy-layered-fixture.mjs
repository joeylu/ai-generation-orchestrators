...
function join(parts) { const result = new Uint8Array(parts.reduce((total, part) => total + part.length, 0)); let offset = 0; for (const part of parts) { result.set(part, offset); offset += part.length; } return result; }
function crc32(bytes) { let value = 0xffffffff; for (const byte of bytes) { value ^= byte; for (let index = 0; index < 8; index += 1) value = (value >>> 1) ^ (value & 1 ? 0xedb88320 : 0); } return (value ^ 0xffffffff) >>> 0; }
function chunk(name, bytes) { const label = encoder.encode(name); return join([be32(bytes.length), label, bytes, be32(crc32(join([label, bytes])))]); }
export function legacyFixturePng() {
...
function zip(entries) {
  const sorted = [...entries].sort((left, right) => left.name < right.name ? -1 : left.name > right.name ? 1 : 0); const locals = [], central = []; let offset = 0;
  for (const entry of sorted) {
    const name = encoder.encode(entry.name), compressed = new Uint8Array(deflateRawSync(entry.bytes)), crc = crc32(entry.bytes), declaredSize = entry.declaredSize ?? entry.bytes.length;
    const local = join([u32(0x04034b50), u16(20), u16(0), u16(8), u16(0), u16(0), u32(crc), u32(compressed.length), u32(declaredSize), u16(name.length), u16(0), name, compressed]);
    locals.push(local); central.push(join([u32(0x02014b50), u16(0x0314), u16(20), u16(0), u16(8), u16(0), u16(0), u32(crc), u32(compressed.length), u32(declaredSize), u16(name.length), u16(0), u16(0), u16(0), u16(0), u32(entry.external ?? 0x81a40000), u32(offset), name])); offset += local.length;
  }
...
/** A fully synthetic compressed ZIP matching the one supported legacy pilot. */
export function legacyLayeredFixtureZip(options = {}) {
  const image = legacyFixturePng();
  const layer = name => ({ kind: 'pixel', name, png: `layers/${name}.png`, sha256: hash(image), rgba_sha256: 'a'.repeat(64), size: [1, 1], left: 0, top: 0, visible: true, opacity: 255, blend_mode: 'normal', alpha: { transparent_pixels: 0, soft_alpha_pixels: 0, opaque_pixels: 1, transparent_nonzero_rgb_pixels: 0 } });
  const inputs = {
  ...
  for (const path of paths) if (path !== 'manifest.json') entries.set(path, encoder.encode(path.endsWith('.png') ? '' : '{}'));
  ...
  const manifest = { kind: 'experimental_ui_layered_export_delivery_v1', status: 'verified_pilot', format: 'psd', fonts_recovered: false, new_generation: false, photoshop_version: '23.4.1', files: paths.filter(path => path !== 'manifest.json').map(path => ({ file: path, bytes: entries.get(path).length, sha256: hash(entries.get(path)) })) };
  ...
  if (options.duplicate) output[0] = { ...output[0], name: 'file-roundtrip.json' };
  if (options.symlink) output[0] = { ...output[0], external: 0xa1ff0000 };
  if (options.oversized) output[0] = { ...output[0], declaredSize: 16 * 1024 * 1024 + 1 };
  return zip(output);
}
