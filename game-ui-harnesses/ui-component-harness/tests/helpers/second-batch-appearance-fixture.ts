...
export async function secondBatchAppearanceFixture() {
  ...
  const fixture = await fixtureLayeredZip([800, 760], [
    ...
    { id: 'tab-a-icon', role: 'important_component', left: 110, top: 430, width: 20, height: 20, color: [70, 75, 65, 255] },
    { id: 'tab-a-active-icon', role: 'important_component', left: 110, top: 430, width: 20, height: 20, color: [245, 245, 225, 255] },
    { id: 'tab-b-icon', role: 'important_component', left: 310, top: 430, width: 20, height: 20, color: [70, 75, 65, 255] },
    { id: 'tab-b-active-icon', role: 'important_component', left: 310, top: 430, width: 20, height: 20, color: [245, 245, 225, 255] },
  ...
  const binding = { kind: 'ui-appearance-binding', version: '0.2', documentSha256: await appearanceDocumentSha256(document), deliveryDigest: imported.deliveryDigest, sceneSha256: imported.sceneSha256, archiveSha256: imported.archiveSha256,
    registration: { sourceCanvas: { width: 800, height: 760 }, targetCanvas: { width: 800, height: 760 }, transform: { scale: 1, offset: { x: 0, y: 0 } } }, bindings: [
      ...
      { componentId: 'apply-tabs', componentType: 'Tabs', parts: [
        ...
        { role: 'icon', layerId: 'tab-a-icon', tabId: 'tab-a' }, { role: 'active-icon', layerId: 'tab-a-active-icon', tabId: 'tab-a' },
        { role: 'icon', layerId: 'tab-b-icon', tabId: 'tab-b' }, { role: 'active-icon', layerId: 'tab-b-active-icon', tabId: 'tab-b' },
      ], states: { tabs: { headerHeight: 40, labelLayout: area('target-item-local', 90, 5, 100, 30), hitArea: area('target-item-local', 0, 0, 200, 40), activeTextColor: '#FFFFFF', icons: [
        { tabId: 'tab-a', iconLayout: area('target-item-local', 60, 10, 20, 20), activeIconLayout: area('target-item-local', 60, 10, 20, 20) },
        { tabId: 'tab-b', iconLayout: area('target-item-local', 60, 10, 20, 20), activeIconLayout: area('target-item-local', 60, 10, 20, 20) },
      ] } } },
