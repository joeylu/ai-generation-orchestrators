...
def receive(run,bundle,strategy_path,revision_path,output,approval):
    ...
    source_regions = report.get('sourceRegions')
    if source_regions is not None:
        # A region revision is self-contained: downstream consumers need not
        # trust a mutable strategy path when reproducing or selecting slots.
        source_strategy = report.get('sourceStrategy')
        require(isinstance(source_strategy, dict), 'REVISION_SOURCE_STRATEGY_REQUIRED')
        verify_strategy(source_strategy)
        require(source_strategy.get('digest') == report.get('original_strategy_digest'),
                'REVISION_SOURCE_STRATEGY_CHANGED')
    ...
    checked=revise(raw,Path(strategy_path),report['board'],report['raw_sha256'],
                   report['extraction_policy'],report['reason'],output,
                   report.get('measuredFrames'), source_regions=source_regions)
    ...
    specification=dict(version='1.0',originalStrategyDigest=checked['original_strategy_digest'],
                       policy=checked['extraction_policy'],reason=checked['reason'],
                       **({'measuredFrames':checked['measuredFrames']} if 'measuredFrames' in checked else {}),
                       **({'sourceRegions':checked['sourceRegions']} if 'sourceRegions' in checked else {}),
                       **({'sourceStrategy':checked['sourceStrategy']} if 'sourceStrategy' in checked else {}))
