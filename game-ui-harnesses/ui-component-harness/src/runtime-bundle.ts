...
export async function validateRuntimeBundle(value:unknown, source:unknown):Promise<UiBundle> {
 ...
 const stable=(b:UiBundle)=>{const c:any=structuredClone(b);delete c.motion;delete c.motionSystem;delete c.bundleVersion;for(const n of stateNodes(c.document))for(const k of mutableState[n.type]??[])delete n.props[k];return canonical(c);};
