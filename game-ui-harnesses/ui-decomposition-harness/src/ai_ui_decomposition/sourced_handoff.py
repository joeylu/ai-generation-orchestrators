...
def prepare_from_sources(compiled,sources,output,component_root):
    ...
    expected={r['generationAsset'] for r in catalog['parts']}
    ...
    for key,source in sources.items():
        ...
        if 'extractionRevision' in source:
            ...
            require(isinstance(revision,dict) and set(revision)=={'version','originalStrategyDigest','policy','reason'} and revision['version']=='1.0','SOURCED_REVISION_VERSION')
            require(source['expectedRawSha256'] is not None,'SOURCED_REVISION_RAW_REQUIRED')
            rows=[r for r in catalog['parts'] if r['generationAsset']==key]
    ...
    for key,(run,item,receipt) in verified.items():
        ...
        if board:
            ...
            if revision:
                ...
                result=revise(raw,strategy,board,sources[key]['expectedRawSha256'],revision['policy'],revision['reason'],output/('extracted-'+board))
