...
for(const mode of ['raster','native','single-frame','no-probe']){
 const rasterOverlay=mode==='raster', singleFrame=mode==='single-frame';
 const dir=resolve(out,mode==='no-probe'?'Dialog-no-probe':rasterOverlay?'Dialog':singleFrame?'Dialog-single-frame':'Dialog-native-overlay');await mkdir(dir);
 ...
 if(mode==='no-probe'){document.root.children=document.root.children.filter(n=>n.id!=='underlay');binding.bindings=binding.bindings.filter(b=>b.componentId!=='underlay');binding.documentSha256=await appearanceDocumentSha256(document);}
