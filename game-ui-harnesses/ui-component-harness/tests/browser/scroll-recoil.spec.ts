...
test('style edge recoil changes paint without fake scroll events or thumb movement',async({page},info)=>{
 ...
 await page.waitForTimeout(16);await page.mouse.wheel(0,20);await page.waitForTimeout(32);await page.waitForTimeout(80);const middle=await canvas.screenshot({path:info.outputPath(style+'-edge.png')});const mid=await snapshot();expect(mid.events.length).toBe(first.events.length);expect((mid.document as any).root.children[0].children[0].props.scrollY).toBe(20);
 ...
 expect(middle.equals(baseline)).toBe(style==='original'||style==='corporate');await page.waitForTimeout(500);const settled=await snapshot();expect(settled.motionSnapshot!.nodes.find(n=>n.id==='scroll')!.presentation.scrollRecoil??0).toBe(0);await canvas.screenshot({path:info.outputPath(style+'-settled.png')});
