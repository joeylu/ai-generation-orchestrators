...
import delivery_dag as delivery
import experimental_executor as exchange
...
class DeliveryTests(unittest.TestCase):
    ...
    def test_authorization_wait_then_delivery_and_idempotent_resume(self):
        ...
        self.complete_media()
        with patch('delivery_dag.register',side_effect=self.fixture_registration) as reg:
            result = self.dag.execute()
    ...
    def test_localization_failure_stops_package_and_no_retry(self):
        ...
        with patch('delivery_dag.register',side_effect=TimeoutError('uncertain model')) as reg:
    ...
    def test_resume_after_registration_does_not_repeat_localization(self):
        ...
        with patch('delivery_dag.register',side_effect=AssertionError('must not rerun')):
