...
1. CI builds a wheel, source distribution, and the implemented thin Skill ZIPs
   from a version tag whose value must match the package and Skill versions.
...
`.github/release_tools/build_release_metadata.py --dist dist` after the Python build. Each
includes only its fixed public Skill allowlist and the project license, with
normalized text and deterministic ZIP metadata. Their SHA-256 values appear in
the same `SHA256SUMS.txt` as the wheel. Extra workspace files are never swept
...
`MANIFEST.in` also includes public documentation, the thin Skills, release tooling,
