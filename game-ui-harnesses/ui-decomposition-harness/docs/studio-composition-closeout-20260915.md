...
用户随后明确要求“只要是正式链路的改动都一起提交，不用只提交本轮”。
因此下一次统一提交覆盖两个UI Harness的剩余正式源码、合同、测试及文档，
不再受上文本次独立提交边界限制。角色动画目录、根目录杂项及work历史产物
仍不在本次UI提交范围。前一独立提交ae58f63保留，测试通过反馈与人工标记不变。
统一回归证据保存在 `work/ui-decomposition/studio-composition-mainline-20260915-r001/`
的closeout日志中；包含消费端构建、离线及浏览器测试，未进行发布或部署。
