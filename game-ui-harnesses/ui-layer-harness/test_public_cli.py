...
class PublicCliTests(unittest.TestCase):
    ...
    def test_version_and_failed_status_are_machine_readable(self):
        entry=Path(__file__).with_name('ui_layer.py')
        result=subprocess.run([sys.executable,str(entry),'--version'],capture_output=True,text=True)
        self.assertEqual(result.returncode,0)
        self.assertEqual(result.stdout.strip(),'0.1.0a1')
        import tempfile
        with tempfile.TemporaryDirectory() as tmp:
            result=subprocess.run([sys.executable,str(entry),'status','--output',tmp],capture_output=True,text=True)
