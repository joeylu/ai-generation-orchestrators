...
2. 切换原样，以及 Playful、Premium、Corporate 三套动效方案，或并排比较四个独立画布；支持重播与真实交互。
...
识图服务属于源码工作台的可选本地适配层，不进入公共安装包。密钥只由本地服务读取；参考图会发送到所配置的识图服务。
拆分链路产物可通过“拆分素材”导入 PNG ZIP，在本地校验并查看原始合成图，
...
下拉选择、进度裁剪、滑块拖动、ScrollView 滚轮/内容拖动/滚动条滑块拖动、列表选择、模态遮罩和页签切换。
...
源码包不含 node_modules、构建缓存、运行报告、服务状态或用户原图。安装包提供 CLI/library；Web 开发请使用源码包。
...
识图适配层、严格合同与独立现场评估已经完成；具体服务商、认证和运行状态不随发布包分发。
新上游产物的稳定跨项目验收、物理触屏与输入法面板、用户美术确认仍未完成。
其他引擎与版本尚未选定，当前只有 PixiJS/Web 通过验收。
不会用程序夹具或整图显示冒充自动识图与可编辑分层成功。
动效验收包含程序夹具的局部像素检查，不代替用户美术质量确认。
旧版 Button 兼容向量仍由源码回归覆盖，但其用户原图与历史记录不随公共包分发。
默认示例使用 `public/fixtures/` 内的程序资源。
